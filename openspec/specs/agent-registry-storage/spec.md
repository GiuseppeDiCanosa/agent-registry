...
# agent-registry-storage Specification

## Purpose
La home del registry può vivere su più macchine tenute allineate da un remote
git privato. Questa capability governa come quella condivisione si stabilisce:
la validazione del remote prima di toccare il filesystem, la scelta fra
inizializzazione, clone e integrazione, le conferme richieste quando ci sono
dati in gioco, e l'identità con cui i commit automatici raccontano da quale
macchina arrivano.
...
**Verified by**: [@test] tests/test_sync_manager.py
