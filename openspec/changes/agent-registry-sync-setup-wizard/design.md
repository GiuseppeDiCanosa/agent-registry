...
- **D7 — Identità git con hostname.** `_ensure_git_identity` imposta `user.email = agent-registry@<hostname>` (da `socket.gethostname()`) invece di `agent-registry@localhost`; i repo già inizializzati non vengono toccati. Rationale: history leggibile multi-macchina a costo zero. Trade-off accettato: due macchine con lo stesso hostname (es. default "MacBook-Pro") restano indistinguibili.
